    ...
    module AmazonSQS
      ...
      class Connection
        ...
        def initialize cfg
            ...
            @sqs = RightAws::Sqs.new(@access_key_id, @secret_access_key, 
               { #:server       => @host, # Amazon service host: 'queue.amazonaws.com' (default)
                :port         => @port ,      # Amazon service port: 80 or 443 (default)
                :multi_thread => cfg[:multithread] || false   ,        # Multi-threaded (connection per each thread): true or false (default)
                :signature_version => cfg[:aws_version] == "2008-01-01" ? 1 : 0 ,              # The signature version : '0' or '1'(default)
                :logger       => logger}  
            )
            queues
        end
        ...
        def send queue_name, message_body, message_headers={}
          queue_name = sanitize_queue_name queue_name
          queue = rightsqs.queue queue_name
        ...
        def create_queue(name)
          ...
          response = rightsqs.queue name
          add_queue name unless response.nil?
        end      	
        ...
        def retrieve_messsages queue, num_messages=1, timeout=nil
          queue.receive_messages (num_messages, timeout)
        end
        ...
        def http_request h, p, r
          return RightHttpConnection.start(:server   => h, :port => p, :request => r)
        ...
        def add_queue(qn)
          queues[qn] = rightsqs.queue qn if self.cache_queue_list
          return qn
        end
      ...
      class Message 
        ...
        def self.from_rightaws_message message
          Message.new(nil, message.id, message.body, message.body, nil, message, message.queue)  
        end
        ...
        def visibility=(visibility_timeout)
          @response.visibility = visibility_timeout
        end
